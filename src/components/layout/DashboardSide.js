...
function DashboardSide({ roule, children, email }) {
  return (
    <div className="flex lg:gap-15 lg:px-6 lg:py-8 gap-4 px-4 max-lg:mt-8 ">
      <div className="flex flex-col items-center  max-lg:w-full p-2 md:p-10  shadow-[0_1px_5px_0_#FFF]">
        <CgProfile className="size-6 lg:size-10" />
        {roule === "ADMIN" ? "ادمین" : null}
        ...
        <Link className="m-1 font-medium w-full" href="/">
          آگهی های من
        </Link>
        <Link className="m-1 font-medium w-full" href="/add-profile">
          ثبت آگهی
        </Link>
        {roule === "ADMIN" ? <Link href="/admin">درانتطار انتشار</Link> : null}
        <LogoutButton />
      </div>
      <div>{children}</div>
    </div>
